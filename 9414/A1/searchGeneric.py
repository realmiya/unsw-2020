...
class GreedySearcher(Searcher):
    # in assign1, add this class to calculate the smallest cost of schedule different tasks
    def __init__(self, problem):
        """
...
class BreadthFirstSearcher(Searcher):
    ...
    def add_to_frontier_state(self,state):
        self.frontier_state.append(state)

    def add_to_expanded_state(self,state):
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            self.display(3,"Neighbors are", neighs)
            for arc in neighs:
                if(arc.to_node not in self.expanded_state) or (arc.to_node in self.frontier_state):
